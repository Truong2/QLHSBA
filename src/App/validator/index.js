...
export function validateRequireInputAcceptZero(message) {
  return {
    required: true,
    validator: (_, value) => {
...
export function validateUrl(message, type) {
  return {
    validator: (_, url) => {
      let index = 0;
      if (type?.length > 0) {
        type?.forEach((el) => {
          if (toLower(url)?.includes(el)) {
            index += 1;
          }
        });
      }
      if (url && trim(url)) {
        ...
        if (
          (!isURL && !isLocalOrAddress) ||
          haveWhitespace ||
          (type?.length > 0 && index === 0)
        ) {
...
// Validate number of cycle when reactive sub
export function validateNumberOfCycleInput(message, max1, max2) {
  const regex = /^[0-9]+$/g;
  return {
    validator: (_, value) => {
      value = value || "";
...
export function formatNormalizeRangeNumber(max, min, value) {
  if (trim(value)) {
    value = value.toString().replaceAll(/\D/g, "");
...
export function formatNormalizeTaxCode(value) {
  if (trim(value)) {
    const taxCode = value.toString().replace(/\s/g, "");
    // let taxCode = '';
...
export function validateStrLength2(max1, max2, message) {
  return {
    validator: (_, value) => {
      value = value || "";
      if (value.indexOf(" ") >= 0) {
...
export function validateLengthTin(max1, max2, message) {
  return {
    validator: (_, value) => {
      value = value.toString() || "";
      if (value && (value.length === max1 || value.length === max2)) {
        return Promise.resolve();
      }
      return Promise.reject(
        message || "Vui lòng chỉ nhập tối đa số ký tự cho phép"
    ...
    validator: (_, value) => {
      if (trim(value)) {
        ...
        if (type === "INCREASE") {
          ...
          return Promise.reject(
            message || `Vui lòng nhập vào giá trị lớn hơn ${valueQuanlity}`
          );
        }
        if (type === "DECREASE") {
          if (parseInt(value, 10) < valueQuanlity) {
          ...
          return Promise.reject(
            message || `Vui lòng nhập vào giá trị nhỏ hơn ${valueQuanlity}`
          );
        }
      }
      return Promise.resolve();
...
export function validateMinMaxInputNumber(min, max, message) {
  return {
    validator: (_, value) => {
      const number = value;
      if (trim(value) && !Number.isNaN(number)) {
        if (min && max) {
          if (number >= min && number <= max) return Promise.resolve();
          return Promise.reject(
            message || `Giá trị được nhập trong khoảng ${min} đến ${max}`
          );
        }
      }
      return Promise.resolve();
    ...
    validator: (_, value) => {
      if (value) {
        ...
        if (isOverLimitSize(value)) {
          return Promise.reject(
            message || "Tổng dung lượng các file không được quá 100MB"
          );
        }
...
export function convertStringVietNamesToEnglish(str) {
  ...
  str = str.trim();
  // convert chữ viết hoa thành viết thường
  str = str.toLowerCase();
